...
public class TaskBar {
    ...
    private static Predicate<Application> validApps;
        ...
        validApps = app -> {
            ...
            if (UltreonDevicesMod.hasAllowedApplications()) {
                if (UltreonDevicesMod.getAllowedApplications().contains(app.getInfo())) {
                    return !UltreonDevicesMod.DEVELOPER_MODE || Settings.isShowAllApps();
                }
                return false;
            } else if (UltreonDevicesMod.DEVELOPER_MODE) {
    ...
    public void handleClick(MineOS mineOS, int x, int y, int mouseX, int mouseY, int mouseButton) {
        if (isMouseInside(mouseX, mouseY, x + 1, y + 1, x + 236, y + 16)) {
            UltreonDevicesMod.LOGGER.debug("Clicked on task bar");
            int appIndex = (mouseX - x - 1) / 16;
            if (appIndex >= 0 && appIndex <= offset + APPS_DISPLAYED && appIndex < mineOS.installedApps.size()) {
        ...
        for (int i = 0; i < trayItems.size(); i++) {
            ...
            if (isMouseInside(mouseX, mouseY, posX, y + 2, posX + 13, y + 15)) {
                ...
                UltreonDevicesMod.LOGGER.debug("Clicked on tray item (%d): %s".formatted(i, trayItem.getClass().getSimpleName()));
                break;
            }
        }
    }
