package dev.ultreon.mineos.apps.system.component;
...
public class AppGrid extends Component {
    ...
    protected void render(GuiGraphics graphics, MineOS laptop, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean windowActive, float partialTicks) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (GuiHelper.isMouseWithin(mouseX, mouseY, itemX, itemY, itemWidth, itemHeight)) {
                ...
                graphics.fill(itemX + 1, itemY + 1, itemX + itemWidth - 1, itemY + itemHeight - 1, MineOS.get().getSettings().getColorScheme().getItemBackgroundColor());
            }
        }
    }
    ...
    private Layout generateAppTile(AppEntry entry, int left, int top) {
        ...
        if (entry instanceof LocalEntry localEntry) {
         ...
         //   dev.ultreon.devices.api.component.app.Image image = new dev.ultreon.devices.api.component.app.Image(iconOffset, padding, 14 * 3, 14 * 3, localEntry.info().getIconU(), localEntry.info().getIconV(), 14, 14, 224, 224, MineOS.ICON_TEXTURES);
        ...
        if (entry instanceof LocalEntry) {
            ...
            if (MineOS.get().getInstalledApplications().contains(info)) {
