...
import dev.ultreon.mineos.apps.system.component.SlideShow;
import dev.ultreon.mineos.apps.system.object.AppEntry;
import dev.ultreon.mineos.apps.system.object.LocalEntry;
import dev.ultreon.mineos.apps.system.object.RemoteEntry;
...
public class LayoutAppPage extends Layout {
    ...
    private dev.ultreon.devices.impl.app.Component imageIcon;
    ...
    public void init() {
        if (entry instanceof LocalEntry) {
            installed = MineOS.get().getInstalledApplications().contains(((LocalEntry) entry).info());
        }

        this.setBackground((graphics, mc, x, y, width, height, mouseX, mouseY, windowActive) ->
        {
            Color color = new Color(MineOS.get().getSettings().getColorScheme().getBackgroundColor());
        ...
        if (entry instanceof LocalEntry) {
          ...
          //  imageIcon = new dev.ultreon.devices.api.component.app.Image(5, 26, 28, 28, info.getIconU(), info.getIconV(), 14, 14, 224, 224, MineOS.ICON_TEXTURES);
        } else if (entry instanceof RemoteEntry) {
